...
| 笔记整理 | 读写笔记、内容摘要、标签分类 |
| 跨应用操作 | 发送邮件、Notion/飞书等（**未实现，Phase 2 / out-of-scope**） |
...
| 可用性 | 桌面常驻、定时 + 事件双触发（系统托盘为 Phase 2 / 未实现） |
...
| 长期记忆 | 文件记忆 + FAISS | `.my-agent/memory/*.md` + 向量库（**未使用** `langgraph.store`） | 用户偏好、跨会话经验 |
...
| 复杂任务规划节点 | 多步骤任务自动分解 |
| 反思节点 | 执行结果自检与重试 |
| 定时触发（Cron / 任务提醒） | 后台提醒、定时任务（无系统托盘） |
