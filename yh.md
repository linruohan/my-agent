...
| `src/ui/controller/turns.py` 等 | ~~宽 except 静默~~ ✅ | turns/clipboard/link + learning_dedupe/metrics/search_cache/tools/ocr 已打 debug |
...
| 位置 | 现状 | 建议 |
|------|------|------|
| `pyproject.toml` | ~~无 ruff~~ ✅ | `dev` extras 含 ruff + mypy（渐进 files） | 可继续扩大 mypy 范围 |
| CI | ~~browser 误跑~~ ✅ | `-m "not browser_integration"` | 已完成 |
| `packaging/stage_release.py` | ~~注释过时~~ ✅ | 标明 app.db 中心 + 旧库跳过 | 已完成 |
| 配置热更新 | ~~需重启~~ ✅ | `/reload` + `reload_config()` + `reload_runtime_config()` | 已完成 |
...
| 同上 — RAG MMR | ~~已注明~~ 当前 top-k；MMR 标 Phase 2 | — |
...
| `memory_promotion` critical 写文件 | ~~无回归~~ ✅ `test_promote_critical_writes_settings_local_not_app_yaml` |

### 其他工程债

| 项 | 说明 |
|----|------|
| `test_search.py` | ~~根目录~~ ✅ 已移至 `scripts/` |
...
| `.gitignore` | ~~已补~~ `app.db` / wal；本地运行时库仍可能脏工作区 |
...
- 非 stub：`service.py`、`inbox.py`、`http_server.py`、三家 bot + webhook。
- 已补：`remote_hitl=ask`、忙时重排队、HTTP webhook；Discord/Slack 更深集成仍可扩展。

### Memory

- 已闭环主路径：双轨互斥、critical→settings.local、settings 合并注入、验证提示、选择缓存+预筛、MemoryService 门面。
- 可选：RAG MMR、Team 记忆更深协作。
...
- 结构清晰：`web/js/core/` + `features/`；`/reload` 热重载配置。
- 残留：语音 UI 未接线（optional `.[speech]`）、系统托盘 Phase 2。

### 配置与数据

- 运行时统一库：`data/app.db`；文档与 packaging 已对齐。
- 热重载：`/reload` / `reload_config()` / `reload_runtime_config()`。
